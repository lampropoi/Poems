...
const List = ({navigation}) => {
  ...
  useEffect(() => {
    const retrivePoems = async () => {
      const retrievedPoems = await getPoems();
      setPoems(retrievedPoems);
    };
  ...
  const ListItem = poem => (
    <TouchableOpacity key={poem.id} onPress={() => navigation.navigate('Poem', {poem})}>
  ...
  return (
    <SafeAreaView style={styles.container}>
      <FlatList data={poems} renderItem={({item}) => <ListItem {...item} />} />
    </SafeAreaView>
  );
};
